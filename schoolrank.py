...
# Load the Excel file and access the Maths and Language results sheets.#file = input('Enter the file directory')
...
# Input the raw data# Fetch the headings
next_cell = ''
headings = []
count = 1
...
next_school = ''
current_cell = ''
count = headings_row + 1# read in all the school names
...
next_school = ''
current_cell = ''
count = headings_row + 1# read in all the grades
for school in data:
...
while next_grade == grade and next_school == school and next_class == Class:
  data[school][grade][Class].update({
    'Teacher': {
      'Name': sheet.cell(row = count, column = 4).value
    },
    \
...
next_cell = sheet.cell(row = headings_row, column = count).value
while next_cell[0] == 'Q'
and next_cell[1].isnumeric():
  if sheet.cell(row = data[school][grade][Class][student]['Number'], column = count).value == '-':
  data[school][grade][Class][student]['Scores'].update({
    sheet.cell(row = headings_row, column = count).value: {
      \
      'Mark': 0,
      \
...
for grade in data[school]:
  ...
  for student in data[school][grade][Class]:
  if student != 'Teacher':
  student_scores[school][student] = {
    'Details': data[school][grade][Class][student]
  }
for question in data[school][grade][Class][student]['Scores']:
  ...
  student_scores[school][student].update({
    data[school][grade][Class][student]['Scores'][question][rank]: [data[school][grade][Class][student]['Scores'][question]['Mark']]
  })
else :
...
for student in student_scores[school]:
  if student != 'Teacher':
  student_averages[school][student] = {
    'Details': student_scores[school][student]['Details']
  }
...
for student in student_averages[school]:
  student_ranks[school][student] = {
    'Details': student_averages[school][student]['Details']
  }
for i in ranks[::-1]:
...
# Combine maths and language averages
for grades and cognitive levels
for each learner
student_averages = {}
for school1 in maths_averages:
  for school2 in language_averages:
  ...
  student_averages[student1] = {
    'Details': maths_averages[school1][student1]['Details'],
    \
    'Maths': {
      'Grade Levels': maths_averages[school1][student1],
      \
    ...
    \
    'Language': {
      'Grade Levels': language_averages[school2][student2],
      \
...
for i in content_language_ticks:
  ...
  student_averages[student2] = {
    'Details': language_averages[school2][student2]['Details'],
    \
    'Maths': {
      'Grade Levels': {
        'Details': ''
...
for i in content_maths_ticks:
  student_averages[student2]['Maths']['Content Domain'][i] = 0# Calculate Averages
...
new_book.save('Grade_10_ML.xlsx' + datetime.datetime.now() )